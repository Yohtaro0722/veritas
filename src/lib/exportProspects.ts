...
export async function buildProspectRows(): Promise<ProspectRow[]> {
  ...
  return tenants
    .filter((t) => isTodayTarget(t.flags))
    .map((t) => ({ t, p: classifyPriority(t.flags) }))
    .sort((a, b) => a.p.rank - b.p.rank || a.t.name.localeCompare(b.t.name))
    .map(({ t, p }) => ({
      優先度: p.label,
      攻め筋: p.strategy,
...
export async function buildProspectsXlsx(): Promise<{ blob: Blob; rows: number }> {
  ...
  // 列幅を読みやすく
  ws['!cols'] = [
    { wch: 14 }, { wch: 30 }, { wch: 22 }, { wch: 20 }, { wch: 6 },
